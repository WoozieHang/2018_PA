...
make_instr_func(mov_rm2s_w){
	...
	len+=modrm_rm_s(eip+1,&opr_src,&opr_dest);
	print_asm_2("mov","w",len,&opr_src,&opr_dest);
	//printf("sreg:%x  ",opr_dest.addr);
	operand_read(&opr_src);
	opr_dest.val = opr_src.val;
	operand_write(&opr_dest);
	//printf("sreg value:%x\n",opr_dest.val);
	//printf("opr_dest.addr:%x",opr_dest.addr);
...
make_instr_func(mov_c2r_l){
	uint32_t modrm_bit=paddr_read(cpu.eip+2,1);
	uint32_t reg=(modrm_bit&0x38)>>3;
	uint32_t creg=modrm_bit&0x07;
	assert(creg==0||creg==3);
	if(creg==0)
	cpu.gpr[reg].val=cpu.cr0.val;
	else
	cpu.gpr[reg].val=cpu.cr3.val;
	return 2;
//	int len=1;
...
//	printf("eip:%x\n",cpu.eip);
	uint32_t modrm_bit=paddr_read(cpu.eip+2,1);
//	printf("modrm_bit:%x\n",modrm_bit);
	uint32_t creg=(modrm_bit&0x38)>>3;
	uint32_t reg=modrm_bit&0x07;
	assert(creg==0||creg==3);
	if(creg==0)
	cpu.cr0.val=cpu.gpr[reg].val;
	else 
	cpu.cr3.val=cpu.gpr[reg].val;
