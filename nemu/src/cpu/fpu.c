...
inline uint32_t internal_normalize(uint32_t sign, int32_t exp, uint64_t sig_grs) {
	...
	if(!overflow) {
		...
		//if carry
		if(((sig_grs&0x07)>0x04)||(((sig_grs&0x08)!=0)&&((sig_grs&0x07)==0x04)))
		{
			sig_grs+=0x08;
		}
		//remove the low three bit
		sig_grs>>=3;
		//if denormal ,shift right once more and increase the exp
		if((sig_grs>>(23))>1)
		{
		sig_grs>>=1;
		exp++;
